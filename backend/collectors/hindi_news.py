...
class HindiNewsCollector:
    ...
    def run(self) -> dict:
        logger.info("Starting Hindi News Collector run...")
        sources_to_process = self.db.query(NewsSource).filter(NewsSource.type == "rss_hindi", NewsSource.is_active == True).all()
