...
class PoliceNewsCollector:
    ...
    def run(self) -> dict:
        logger.info("Starting Police News Collector run...")
        sources_to_process = self.db.query(NewsSource).filter(NewsSource.type == "rss_police", NewsSource.is_active == True).all()
