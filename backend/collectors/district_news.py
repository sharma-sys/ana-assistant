...
class DistrictNewsCollector:
    ...
    def run(self) -> dict:
        logger.info("Starting District News Collector run...")
        sources_to_process = self.db.query(NewsSource).filter(NewsSource.type == "rss_district", NewsSource.is_active == True).all()
