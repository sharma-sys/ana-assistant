...
class NationalNewsCollector:
    ...
    def run(self) -> dict:
        logger.info("Starting National News Collector run...")
        sources_to_process = self.db.query(NewsSource).filter(NewsSource.type == "rss_national", NewsSource.is_active == True).all()
